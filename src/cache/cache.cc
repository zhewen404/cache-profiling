...
////////////////////////////////////////
u_int8_t *
ClusteringManager::get_line_true_fingerprint(Line * line, int& true_fingerprint_size) const
{
...
u_int8_t * 
ClusteringManager::compute_cascaded_fingerprint_for_a_line(Line * line, int& fingerprint_size)
{
    u_int8_t * fingerprint;
...
u_int8_t * 
ClusteringManager::compute_cascaded_fingerprint_for_a_line_true(Line * line, int& fingerprint_size) const
{
...
u_int8_t * 
ClusteringManager::compute_concact_fingerprint_for_a_line(Line * line, int& fingerprint_size)
{
    u_int8_t * fingerprint;
    fingerprint_size = 0;
    // m_hash_functions[0]->print();
...
u_int8_t * 
ClusteringManager::compute_concact_fingerprint_for_a_line_true(Line * line, int& fingerprint_size) const
{
    int num_funct_executed = 0;
    u_int8_t * fingerprint;
    fingerprint_size = 0;
...
u_int8_t *
ClusteringManager::get_fingerprint_of_a_line(Line * line, int& fingerprint_size)
{
    u_int8_t * fingerprint;
    if (m_cascade_hash_functions) {
...
void 
ClusteringManager::print()const
{
    printf("ClusteringManager [ cascade_hash_functions: %d, num_hash_functions: %d, num_funct_true_fingerprint: %d, funct_to_concact: %d ]\n", 
        m_cascade_hash_functions, m_num_hash_functions, m_num_funct_true_fingerprint, m_funct_to_concact);
    for (unsigned i = 0; i < m_hash_functions.size(); i++) {
        m_hash_functions[i]->print();
    }
}
...
ClusteredCache::populate_clusters()
{
    ...
    if (m_clustering_manager->m_num_hash_functions == 0 || 
        m_clustering_manager->m_hash_functions.empty() || 
        m_clustering_manager->m_hash_functions[0] == nullptr) {
    ...
    for (int i = 0; i < m_num_banks; i++) {
        for (int j = 0; j < m_num_sets; j++) {
            for (long unsigned int k = 0; k < m_lines[i][j].size(); k++) {
                ...
                fingerprint = m_clustering_manager->get_fingerprint_of_a_line(line, fingerprint_size);
...
{
    printf("ClusteredCache [ num_banks: %d, KB_per_bank: %d, assoc: %d, numset: %d, line_size: %d, shift_bank: %d, shift_set: %d, bank bits: [%d,%d], set bits: [%d,%d], num_clusters: %lu ]\n", 
        m_num_banks, m_size_per_bank_KB, m_assoc, m_num_sets, m_line_size, m_shift_bank, m_shift_set, m_bank_start, m_bank_end, m_set_start, m_set_end, m_num_clusters);
    printf("bit entropy: %f\n", get_bit_entropy());
    m_clustering_manager->print();
...
////////////////////////////////////////
void
BankedClusteredCache::populate_clusters()
{
    if (!m_lines_populated) {
        printf("Error: lines not populated yet\n");
        return;
    ...
    // iterate through all lines
    for (int i = 0; i < m_num_banks; i++) {
        unordered_map<u_int64_t, int> this_bank_fingerprint_to_cluster = unordered_map<u_int64_t, int>();
        vector<vector<Line*>> this_bank_clusters = vector<vector<Line*>>();
        for (int j = 0; j < m_num_sets; j++) {
            for (long unsigned int k = 0; k < m_lines[i][j].size(); k++) {
                Line * line = m_lines[i][j][k];
                u_int8_t * fingerprint;
                ...
                // translate fingerprint to cluster index
                int id = -1;
                if (this_bank_fingerprint_to_cluster.find(fp64) == this_bank_fingerprint_to_cluster.end()) {
                    // did not find match
                    id = this_bank_clusters.size();
                    this_bank_fingerprint_to_cluster[fp64] = id; // add to map
                    this_bank_clusters.push_back(vector<Line*>());
                } else{
                    id = this_bank_fingerprint_to_cluster[fp64];
                }

                this_bank_clusters[id].push_back(line);
                delete [] fingerprint;
            }
        }
        m_bank_clusters.push_back(this_bank_clusters);
        m_bank_fingerprint_to_cluster.push_back(this_bank_fingerprint_to_cluster);
    }
    m_clusters_populated = true;
}
void
BankedClusteredCache::populate_lines(vector<string> filenames_data, vector<string> filenames_addr)
{
    Cache::populate_lines(filenames_data, filenames_addr);
    populate_clusters();
}
void 
BankedClusteredCache::print()
{
    printf("BankedClusteredCache [ num_banks: %d, KB_per_bank: %d, assoc: %d, numset: %d, line_size: %d, shift_bank: %d, shift_set: %d, bank bits: [%d,%d], set bits: [%d,%d], num_clusters: %lu ]\n", 
        m_num_banks, m_size_per_bank_KB, m_assoc, m_num_sets, m_line_size, m_shift_bank, m_shift_set, m_bank_start, m_bank_end, m_set_start, m_set_end, m_num_clusters);
    printf("bit entropy: %f\n", get_bit_entropy());
    m_clustering_manager->print();
    if (!m_lines_populated || !m_clusters_populated) {
    ...
    for (unsigned i = 0; i < m_bank_clusters.size(); i++) {
        for (unsigned j = 0; j < m_bank_clusters[i].size(); j++) {
            printf("bank[%d] cluster[%d] size: %ld\n", i, j, m_bank_clusters[i][j].size());
        }
    }
}
